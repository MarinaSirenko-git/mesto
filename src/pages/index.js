  ...
  const instanceCard = new Card(
    {
      ...
      likeAddTracker: (isLike) => {
        if (isLike) {
          api
            .removeLike(instanceCard.getId())
            .then(instanceCard.removeLike())
            .catch((error) =>
              console.log(`Ошибка ${error} при удалении лайка`)
            );
        } else {
          api
            .doLike(instanceCard.getId())
            .then(instanceCard.addLike())
            .catch((error) =>
...
const userPopup = new PopupWithForm({
  ...
  handleFormSubmit: (inputValues) => {
    ...
    api
      ...
      .catch((error) =>
        console.log("Ошибка при отправке информации о пользователе")
      )
      .finally(() => {
